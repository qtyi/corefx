#
# This script controls the CoreFx build process. This encompasess everything from build, testing to
...
function Print-Usage() {
  ...
  Write-Host "  -buildServerLog           Create CoreFx build server log"
  Write-Host ""
  Write-Host "Actions:"
  ...
  Write-Host "  -testUsedAssemblies       Run extra checks to validate used assemblies feature (see CoreFx_TEST_USEDASSEMBLIES in codebase)"
  Write-Host ""
...
function BuildSolution() {
  ...
  if ($buildServerLog) {
    ${env:CoreFxCOMMANDLINELOGFILE} = Join-Path $LogDir "Build.Server.log"
  }
  ...
  $CoreFxUseHardLinks = if ($ci) { "/p:CoreFxUSEHARDLINKS=true" } else { "" }
  ...
  try {
    ...
    ${env:CoreFxCOMMANDLINELOGFILE} = $null
  }
}
...
function TestUsingRunTests() {
  ...
  if ($ci) {
    $env:CoreFx_TEST_CI = "true"
  }

  if ($testIOperation) {
    $env:CoreFx_TEST_IOPERATION = "true"
  }

  if ($testUsedAssemblies) {
    $env:CoreFx_TEST_USEDASSEMBLIES = "true"
  }
  ...
  try {
    ...
    if ($ci) {
      Remove-Item env:\CoreFx_TEST_CI
    }
    ...
    if ($testIOperation) {
      Remove-Item env:\CoreFx_TEST_IOPERATION
    }

    if ($testUsedAssemblies) {
      Remove-Item env:\CoreFx_TEST_USEDASSEMBLIES
    }
...
function Setup-IntegrationTestRun() {
  ...
  $env:CoreFx_OOP64BIT = "$oop64bit"
  $env:CoreFx_OOPCORECLR = "$oopCoreClr"
  $env:CoreFx_LSPEDITOR = "$lspEditor"
}
...
try {
  ...
  if ($launch) {
    ...
    &$devenvExe /rootSuffix CoreFxDev
  }
