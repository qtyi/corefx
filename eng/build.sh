#!/usr/bin/env bash
# Copyright (c) Qtyi and contributors. All rights reserved.
...
usage()
{
  ...
  echo "  --solution                 Soluton to build (Default is CoreFx.sln)"
  echo ""
...
function BuildSolution {
  ...
  if [[ "$binary_log" = true ]]; then
    ...
    export LunaCommandLineLogFile="$log_dir/vbcscompiler.log"
  fi
  ...
  if [[ "$test_ioperation" == true ]]; then
    export LUNA_TEST_IOPERATION="true"
  ...
  local luna_use_hard_links=""
  if [[ "$ci" == true ]]; then
    luna_use_hard_links="/p:LUNAUSEHARDLINKS=true"
  fi
