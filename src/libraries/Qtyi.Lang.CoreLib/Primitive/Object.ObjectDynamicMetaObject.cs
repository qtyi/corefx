...
partial class Object
{
    protected class ObjectDynamicMetaObject : DynamicMetaObject
    {
        ...
        {
            if (!this.HasValue) throw new NullReferenceException();

            var invokeMethod = this.Value!.GetType().GetMethod("Invoke");
            Debug.Assert(invokeMethod is not null);
            return Expression.Call(instance, invokeMethod, args);
        }
    }
}
